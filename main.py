...
def gg(audio_files):
    ...
    if audio_files:
        ...
        fragments = split_audio(os.path.join(output_directory, f'converted_{os.path.basename(shortest_file)}'),
                                output_directory, fragment_duration)
        ...
        for audio in audio_files:
            ...
            for i, fragment in enumerate(fragments):
                ...
                fragment_start_time = len(
                    AudioSegment.from_file(fragment_path)) / 1000  # Время начала фрагмента в секундах
                fragment_end_time = fragment_start_time + len(
                    audio_fragment) / 930  # Время окончания фрагмента в секундах
                if text == 0 and audio_fragment == 0 or (fragment_end_time - fragment_start_time) > 15.0 or (
                        fragment_end_time - fragment_start_time) < 12.0:
...
root["bg"] = "gray"
logo_image = PhotoImage(file='image.png')
photo = ttk.Label(root, image=logo_image)
photo.place(x=10, y=20, width=100, height=100)
Zagolovok = ttk.Label(root, text="AudioFile changing", font="Arial 45", background="gray").place(x=120, y=40,
...
Finish = ttk.Label(root, text="Результат:", font="Arial 30", background="gray").place(x=250, y=380, width=400,
                                                                                      height=100)

paus = ttk.Button(text="Удалить\n паузы", command=remove_silence_off).place(x=10, y=390, height=55, width=100)

transkrib = ttk.Button(text="Транскрибировать\n аудиофайл", command=tran_audio).place(x=1080, y=270, height=55,
                                                                                      width=200)
